...
const DieCutBlockPDF: React.FC<DieCutBlockPDFProps> = ({
  onClose,
  ...
  const faca = so.dieCutBlockDetails?.origin
    ?.map((origin: string) => getLabelFromValue(origin, originOptions))
    .join(", ");
          ...
          <div className="w-full border-[2px] border-black bg-white p-2">
            ...
            <div className="bg-neutral-50 p-1 rounded-xl space-y-2">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <InfoField label={"Arquivo:"} value={file || printSheet} />
              </div>
            </div>

            <h2 className="text-lg text-center lg:text-left mt-4">Montagem</h2>

            <div className=" bg-neutral-50 p-1 rounded-xl">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <InfoField label="Impressora:" value={so?.printer?.name} />

                <InfoField label="Faca origem:" value={faca} />
                <InfoField
                  label="Vista:"
                  value={getLabelFromValue(
                    so.dieCutBlockDetails?.view,
                    viewOptions,
                  )}
                />
                <InfoField label="PO:" value={so.dieCutBlockDetails?.po} />
                <InfoField
                  label="Sentido onda:"
                  value={getLabelFromValue(
                    so.dieCutBlockDetails?.waveDirection,
                    waveDirectionOptions,
                  )}
                />
                <InfoField
                  label="Largura caixa:"
                  value={so.dieCutBlockDetails?.boxWidth}
                />
                <InfoField
                  label="Altura caixa:"
                  value={so.dieCutBlockDetails?.boxHeight}
                />
                <InfoField
                  label="Qtd. peças largura:"
                  value={so.dieCutBlockDetails?.piecesAmountInWidth}
                />
                <InfoField
                  label="Qtd. peças altura:"
                  value={so.dieCutBlockDetails?.piecesAmountInHeight}
                />
                <InfoField
                  label="Peças p/ forma:"
                  value={so.dieCutBlockDetails?.piecesAmount}
                />
              </div>
            </div>

            <h2 className="text-lg text-center lg:text-left mt-4">Medidas</h2>
            ...
            <div className="flex justify-around gap-2 mt-6">
              <div>
                ...
                <div className="relative border rounded-[10px] border-black p-2 h-[145px] w-[230px] overflow-y-auto">
