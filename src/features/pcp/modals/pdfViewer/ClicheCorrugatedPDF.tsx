  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 items-center gap-1 min-w-0">
        <div
          className={`px-1 min-h-[18px] flex items-center justify-between border rounded-[4px] border-black bg-white text-black text-left text-xs w-full overflow-hidden ${className || ""}`}
          style={{
            whiteSpace: "nowrap",
        ...
        >
          <span
            className={`truncate flex-1 ${shouldHide ? "blur-sm select-none print:hidden" : ""}`}
          ...
          {suffix && (
            <span className="text-xs text-black ml-1 shrink-0">{suffix}</span>
          )}
        </div>
...
const ClicheCorrugatedPDF: React.FC<ClicheCorrugatedPDFProps> = ({
  onClose,
  ...
  const profileName = profiles[0]?.name as string;
  const nfNumber = so.invoiceDetails?.nfNumber;
  const purchaseOrder =
    so?.invoiceDetails?.purchaseOrder || so.purchaseOrder || "";
  const imageProcessing = so.printerDetails?.imageProcessing;
  const finalArt = so.printerDetails?.finalArt;
  const easyflow = so.printerDetails?.easyflow;
  const easyflowFlowType = getLabelFromValue(
    so.printerDetails?.easyflowFlowType,
    easyflowTypeOptions,
  );
  const quantityPrinter = so.printerDetails?.quantityPrinter;
  const quantityProfileTest = so.printerDetails?.quantityProfileTest;
  const printingSide = getLabelFromValue(
    so.printerDetails?.printingSide,
    printingSideOptions,
  );
    ...
    .pdf-content-wrapper {
      ...
      z-index: 2 !important;
        ...
        <div
          ...
          className="pdf-content-wrapper mx-auto rounded-[12px] border-[2px] border-gray-300 p-1 bg-white text-black text-xs"
        >
          <div className="w-full border-[2px] border-black bg-white p-2">
            ...
            <div className="bg-neutral-50 p-1 rounded-xl space-y-2">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <InfoField label={"Arquivo:"} value={printSheet} />
              </div>
            </div>
            ...
            <div className=" bg-neutral-50 p-1 px-[2px] rounded-xl flex justify-center gap-2 items-center flex-wrap">
              {!easyflow ? (
                <div className="flex items-center justify-center">
                  ...
                  <input
                    type="checkbox"
                    className="custom-checkbox ml-[2px]"
                    checked={easyflow}
                    readOnly
                  />
                </div>
                ...
                <div className="flex items-center justify-center">
                  ...
                  <div className="bg-black text-white px-2 py-0.5 rounded text-xs ml-[2px]">
                    {easyflowFlowType}
                  </div>
                </div>
              )}
              ...
              <div className="flex items-center justify-center">
                <label className="block text-xs font-semibold">Printer:</label>
                <div className="bg-black text-white px-2 py-0.5 rounded text-xs ml-[2px]">
                  {quantityPrinter ?? 0}
                </div>
              </div>
              ...
              {printingSide && (
                <div className="flex items-center justify-center">
                  <label className="block text-xs font-semibold">Camada:</label>
                  <div className="bg-black text-white px-2 py-0.5 rounded text-xs ml-[2px]">
                    {printingSide}
                  </div>
                </div>
              ...
              <div className="flex items-center justify-center">
                <label className="block text-xs font-semibold">Jogos:</label>
                <div className="bg-black text-white px-2 py-0.5 rounded text-xs ml-[2px]">
            ...
            <div className="grid grid-cols-3 gap-2 mt-1">
              ...
              <div>
                <label className="block text-sm">Ficha de forma:</label>
                <PdfPreview
                  serviceOrderId={so.id}
                  fileType="dieCutBlockSheet"
                />
              </div>
...
export default ClicheCorrugatedPDF;
